...
export function TenantsPage() {
  ...
  const closeForm = () => {
    ...
    setEditing(undefined); setForm(emptyForm); setFormError('')
  }
  const save = async () => {
    if (savePending.current) return
    const validationError = editing ? '' : validateWorkspaceAdminAccount(form.username, form.password)
    if (validationError || !validRoomCode(form.room_code) || form.room_name.trim().length < 2) {
      setFormError(validationError || (!validRoomCode(form.room_code) ? '房间号须为 5–12 位数字' : '房间名称至少 2 个字符'))
      return
    }
    savePending.current = true
    setSaving(true); setFormError('')
    try {
      const payload = { ...form, username: form.username.trim(), room_code: form.room_code.trim(), room_name: form.room_name.trim() }
      if (editing) await adminApi.updateTenant(editing.id, payload)
      else {
        const account = await adminApi.createTenant(payload)
        setCreatedAdmin(createdWorkspaceAdmin('tenant', account))
      }
      showMessage(editing ? '租户资料已保存' : '租户账号已创建'); setEditing(undefined); setForm(emptyForm); await load()
  ...
  return <Box p={{ xs: 1.5, md: 2.5 }}>
    <Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" alignItems={{ xs: 'stretch', sm: 'center' }} gap={1.5} mb={1.5}>
      <Typography variant="body2" color="text.secondary">租户账号、联系方式和密码统一在这里维护，直属房间作为账号的关联配置。</Typography>
      <Button variant="contained" startIcon={<AddRounded />} onClick={() => open()}>开通租户账号</Button>
    </Stack>
