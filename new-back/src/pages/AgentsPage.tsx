...
export function AgentsPage() {
  ...
  const saveNew = async () => {
    if (!form.username.trim() || new TextEncoder().encode(form.password).length < 8 || !validRoom(form.room_code)) { setError('请填写登录账号、8–72 位密码和 5–12 位数字房间号'); return }
    setSaving(true)
  ...
  const saveEdit = async () => {
    if (!editing) return
    if (!validRoom(form.room_code)) { setError('房间号须为 5–12 位数字'); return }
    setSaving(true)
  ...
  const promoteExisting = async () => {
    ...
    if (!id || !validRoom(promote.room_code)) { setError('请填写已有用户 ID 和 5–12 位数字房间号'); return }
    setSaving(true)
  ...
  return <Box p={{ xs: 2, lg: 2.5 }}>
    <PageHeader eyebrow="业务管理 / 代理" title="代理管理" description="" actions={<><Button variant="outlined" startIcon={<PersonAddAlt1Rounded />} onClick={() => setPromoteOpen(true)}>已有用户转代理</Button><Button variant="contained" startIcon={<AddRounded />} onClick={openCreate}>新增代理</Button></>} />
    ...
    <Dialog open={promoteOpen} onClose={() => !saving && setPromoteOpen(false)} fullWidth maxWidth="xs"><DialogTitle>已有用户转为代理</DialogTitle><DialogContent><Alert severity="info" sx={{ mt: 1, mb: 2 }}>输入用户管理里的内部用户 ID。转换后，该用户可继续用原登录账号和密码进入会员端。</Alert><Stack gap={1.5}><TextField required type="number" label="已有用户 ID" value={promote.user_id} onChange={event => setPromote(current => ({ ...current, user_id: event.target.value }))} /><TextField required label="分配房间号" value={promote.room_code} onChange={event => setPromote(current => ({ ...current, room_code: event.target.value.replace(/\D/g, '') }))} helperText="5–12 位数字且不可与其他代理重复" /></Stack></DialogContent><DialogActions><Button onClick={() => setPromoteOpen(false)}>取消</Button><Button variant="contained" disabled={saving} onClick={() => void promoteExisting()}>{saving ? '处理中…' : '确认转换'}</Button></DialogActions></Dialog>
