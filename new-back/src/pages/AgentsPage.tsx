...
const emptyAgent = () => ({ username: '', password: '', nickname: '', email: '', phone: '', room_code: '', room_name: '', room_logo: '', rebate_rate: 0, profit_share_rate: 0, robot_quota: 0, remark: '', status: 1, tenant_id: 0 })
...
export function AgentsPage() {
  ...
  const saveEdit = async () => {
    ...
    creating.current = true
    setSaving(true); setFormError('')
    try { await adminApi.updateAgent(editing.id, { email: form.email, nickname: form.nickname, phone: form.phone, room_code: form.room_code.trim(), room_name: form.room_name.trim(), room_logo: form.room_logo, rebate_rate: form.rebate_rate, profit_share_rate: form.profit_share_rate, robot_quota: form.robot_quota, remark: form.remark, status: form.status, tenant_id: form.tenant_id || undefined }); showMessage('代理资料、租户归属与房间配置已保存'); setEditing(null); setForm(emptyAgent()); await load() }
