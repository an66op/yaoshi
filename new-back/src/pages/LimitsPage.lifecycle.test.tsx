import { Alert, Button, Chip, Dialog, DialogTitle, TableBody, TableRow } from '@mui/material'
import { isValidElement, type DependencyList, type ReactElement, type ReactNode, type SetStateAction } from 'react'
import { afterEach, beforeEach, describe, expect, it, vi } from 'vitest'
import type { AdminGame, GameOddsLimits, PlayCatalogItem, PlayLimitItem } from '../api'
...
const runtime = vi.hoisted(() => ({
  ...
  resetOddsLimits: vi.fn(), syncOddsLimits: vi.fn(), showMessage: vi.fn(), confirm: vi.fn(),
}))
...
vi.mock('../api', () => ({ adminApi: {
  ...
  updateOddsLimits: runtime.updateOddsLimits, resetOddsLimits: runtime.resetOddsLimits, syncOddsLimits: runtime.syncOddsLimits,
} }))
vi.mock('../components/feedback', () => ({ useFeedback: () => ({ showMessage: runtime.showMessage }) }))

type Props = {
  children?: ReactNode; actions?: ReactNode; label?: string; severity?: string; open?: boolean; disabled?: boolean
  gameId?: string; games?: AdminGame[]; items?: PlayLimitItem[]; catalog?: Record<string, PlayCatalogItem>
  onClick?: () => void; onClose?: () => void; onSelect?: (id: string) => void; onChange?: (items: PlayLimitItem[]) => void
}
...
const unknown = game('pc-canada', 'PC加拿大')
const stopped = game('official-fc3d', '福彩3D', false)
const catalogItem = (play_code: string, play_name: string, category: string, description: string, example: string, default_odds = 1.993): PlayCatalogItem => ({
  play_code, play_name, category, description, example, default_odds, sort_order: 0,
})
const racingCatalog = [
  catalogItem('ball_1_5', '指定名次号码', '号码', '第1–10名号码1–10', '1/0/20', 9.9),
...
const digitsCatalog = [
  catalogItem('ball_1_5', '指定球位号码', '号码', '第1–5球号码0–9', '1/0/20', 9.9),
  catalogItem('two_sided', '两面', '两面盘', '每个球位的大小单双', '5/大/20'),
  catalogItem('dragon_tiger', '龙虎', '龙虎', '首尾对应球位比较', '1/龙/20'),
  catalogItem('sum', '总和 / 总和尾', '总和', '全部5球总和及个位数', '总和/大/20、总和尾/7/20'),
  ...['豹子', '顺子', '对子', '半顺', '杂六'].map((name, index) => catalogItem(['leopard', 'straight', 'pair', 'half_straight', 'mixed'][index], `前三${name}`, '前三形态', '仅前三球，不支持中三和后三', `${name}/20`, [50, 15, 8, 6, 4][index])),
]
const itemsFor = (catalog: PlayCatalogItem[]): PlayLimitItem[] => catalog.map(item => ({
  play_code: item.play_code, play_name: item.play_name, odds: item.default_odds,
...
const limitsFor = (selected: AdminGame): GameOddsLimits => ({
  ...
  rule_version: selected.id === unknown.id ? '' : selected.id === racing.id ? 'racing-v2' : 'digits5-v2',
...
beforeEach(() => {
  ...
  runtime.updateOddsLimits.mockImplementation(async (id: string, items: PlayLimitItem[]) => ({ ...limitsFor(allGames.find(item => item.id === id)!), items }))
  runtime.resetOddsLimits.mockImplementation(async (id: string) => limitsFor(allGames.find(item => item.id === id)!))
  runtime.syncOddsLimits.mockResolvedValue({ game_count: 16, seeded_games: [] })
...
describe('per-game platform odds lifecycle', () => {
  it('distinguishes an audited backend save from a legacy price awaiting confirmation', () => {
    const catalog = Object.fromEntries([
      catalogItem('sum_big', '冠亚和大', '冠亚和', '冠亚和大', '冠亚/大/20', 2.18),
      catalogItem('sum_3', '冠亚和3', '冠亚和', '冠亚和3', '冠亚/3/20', 42.5),
    ...
    expect(chips(root)).toContain('待后台确认')
  })
  ...
  it('shows saved odds risk without overwriting values, and clears it after a safe save', async () => {
    ...
    root = await settle()
    expect(runtime.updateOddsLimits).toHaveBeenCalledExactlyOnceWith(digits.id, edited)
  ...
  it('does not carry a previous game risk into another game or hide a warning after reset', async () => {
    ...
    let root = await select(await ready(), digits.id)
    button(root, '恢复当前').props.onClick!()
    root = await settle()
    expect(alerts(root, 'warning').join('')).toContain(warning.message)
  ...
  it('switches the guide and grid to SSC totals and front-three shapes, never racing crown sums', async () => {
    let root = await select(await ready(), digits.id)
    ...
    expect(grid(root).props.catalog!.sum).toMatchObject({ play_name: '总和 / 总和尾', description: '全部5球总和及个位数' })
    expect(grid(root).props.items).toHaveLength(9)
    button(root, '玩法说明').props.onClick!()
    root = render()
    const guide = ofType(root, Dialog)[0]
    expect(text(guide)).toContain('极速时时彩 · 玩法说明')
    expect(text(guide)).toContain('总和尾/7/20')
    expect(text(guide)).toContain('前三豹子')
    expect(text(guide)).not.toContain('冠亚')
  })
  ...
  it('keeps any retained legacy rows read-only when the server explicitly marks rules unavailable', async () => {
    ...
    expect(button(root, '恢复当前').props.disabled).toBe(true)
    expect(runtime.updateOddsLimits).not.toHaveBeenCalled()
    expect(runtime.resetOddsLimits).not.toHaveBeenCalled()
  })
  ...
  it('waits for both endpoints and ignores an old catalog that finishes after switching games', async () => {
    ...
    expect(grid(root).props.items).toEqual(limitsFor(digits).items)
    expect(grid(root).props.catalog!.sum.play_name).toBe('总和 / 总和尾')
  ...
  it('saves edited values to the selected game and freezes navigation and editing until completion', async () => {
    ...
    expect(button(root, '恢复当前').props.disabled).toBe(true)
    expect(button(root, '补全彩种').props.disabled).toBe(true)
    navigation(root).props.onSelect!(digits.id)
    grid(root).props.onChange!([])
    root = render()
    expect(navigation(root).props.gameId).toBe(racing.id)
    expect(grid(root).props.items).toEqual(edited)
    expect(runtime.updateOddsLimits).toHaveBeenCalledExactlyOnceWith(racing.id, edited)
  ...
  it.each(['save', 'reset', 'sync'] as const)('%s locks same-frame duplicate writes and stale game-selection callbacks', async operation => {
    const pending = deferred<GameOddsLimits | { game_count: number; seeded_games: string[] }>()
    const operationMock = operation === 'save' ? runtime.updateOddsLimits : operation === 'reset' ? runtime.resetOddsLimits : runtime.syncOddsLimits
    operationMock.mockReturnValue(pending.promise)
    const root = await ready()
    const label = operation === 'save' ? '保存设置' : operation === 'reset' ? '恢复当前' : '补全彩种'
    button(root, label).props.onClick!()
    // Before a render, every closure still has saving=false and busy=null.
    navigation(root).props.onSelect!(digits.id)
    for (const staleLabel of ['保存设置', '恢复当前', '补全彩种']) button(root, staleLabel).props.onClick!()
    grid(root).props.onChange!([])
    let current = render()
    ...
    expect(runtime.updateOddsLimits.mock.calls.length + runtime.resetOddsLimits.mock.calls.length + runtime.syncOddsLimits.mock.calls.length).toBe(1)
    pending.resolve(operation === 'sync' ? { game_count: 16, seeded_games: [] } : limitsFor(racing))
  ...
  it.each(['save', 'reset', 'sync'] as const)('releases the lock after failed %s, preserves edits and allows another game to load', async operation => {
    const operationMock = operation === 'save' ? runtime.updateOddsLimits : operation === 'reset' ? runtime.resetOddsLimits : runtime.syncOddsLimits
    ...
    grid(root).props.onChange!(edited)
    root = render()
    button(root, operation === 'save' ? '保存设置' : operation === 'reset' ? '恢复当前' : '补全彩种').props.onClick!()
    root = await settle()
    expect(grid(root).props.items).toEqual(edited)
