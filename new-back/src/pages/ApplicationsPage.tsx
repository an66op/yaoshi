...
export function ApplicationsPage({ initialCategory = 'wallet' }: { initialCategory?: ApplicationCategory }) {
  const [category, setCategory] = useState<ApplicationCategory>(initialCategory)
  ...
  const [applied, setApplied] = useState({ query: '', status: 'all', type: initialCategory, date: '' })
  ...
  const openCreate = async () => {
    setError('')
    setForm({ ...emptyForm, game_id: entertainmentOnly ? '' : undefined })
    setFormOpen(true)
    try {
  ...
  const applyFilters = () => { setPage(0); setApplied({ query: query.trim(), status, type: category, date }) }
  const resetFilters = () => { setQuery(''); setStatus('all'); setDate(''); setPage(0); setApplied({ query: '', status: 'all', type: category, date: '' }) }
  const selectCategory = (next: ApplicationCategory) => {
    ...
    setDate('')
    setPage(0)
    setApplied({ query: '', status: 'all', type: next, date: '' })
  }
  const exportCsv = () => {
