  ...
  return <Box p={{ xs: 1.5, md: 2.5 }}>
    ...
    <Card sx={{ mt: 2 }}><CardContent sx={{ p: { xs: 2, md: 2.5 } }}>
      <Stack direction="row" alignItems="center" gap={1.25} mb={2} flexWrap="wrap">
		...
		<Box flex={1}><Typography fontWeight={850}>房间自动下注</Typography><Typography variant="caption" color="text.secondary">上级已分配 {setting?.robot_quota ?? 0}/10 个机器人 · 今日 {setting?.today_bets ?? 0}/{setting?.daily_bet_limit ?? 200} · 待结 {setting?.pending_bets ?? 0}/{setting?.max_pending_bets ?? 50}</Typography></Box>
      ...
      {loading || !setting ? <CircularProgress size={22} /> : <Stack gap={2}>
        {setting.robot_quota === 0 && <Alert severity="warning">上级尚未分配机器人名额，请联系上级分配后再启用。</Alert>}
        <FormControlLabel control={<Switch disabled={setting.robot_quota === 0} checked={setting.enabled} onChange={event => setSetting(current => current ? { ...current, enabled: event.target.checked } : current)} />} label="开启本房间机器人" />
        ...
        <Stack direction="row" gap={1} flexWrap="wrap">
          <Button variant="contained" disabled={saving || setting.robot_quota === 0} onClick={() => void save()}>{saving ? '保存中…' : '保存设置'}</Button>
          <Button variant="outlined" startIcon={<PlayArrowRounded />} disabled={saving || setting.robot_quota === 0} onClick={() => void runOnce()}>立即执行一轮</Button>
        </Stack>
	...
	<Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: 'repeat(2,1fr)', xl: 'repeat(3,1fr)' }, gap: 1.25, mt: 1.5 }}>
		...
		{!loading && !robots.length && <Alert severity="info">{setting?.robot_quota === 0 ? '上级尚未分配机器人名额。' : '当前分配名额内暂无机器人账号。'}</Alert>}
	</Box>
