...
import { isRacingPlanGame, planRecommendationNumberError, planRecommendationNumberRule, type PlanRecommendationDraft } from '../utils/planRecommendation'
...
export function PlanRecommendationNumberFields({ gameId, value, onChange }: { gameId: string; value: NumberFieldValues; onChange: (patch: Partial<NumberFieldValues>) => void }) {
  const racing = isRacingPlanGame(gameId)
  const error = value.numbersText.trim() ? planRecommendationNumberError(gameId, value.numbersText) : ''
  return <>
    <TextField label="推荐号码" error={Boolean(error)} helperText={error || planRecommendationNumberRule(gameId)} value={value.numbersText} onChange={event => onChange({ numbersText: event.target.value })} />
