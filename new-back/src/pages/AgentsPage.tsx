...
import { adminApi, type AdminGame, type AgentItem, type AgentListResponse, type RoomTradingConfig } from '../api'
...
const emptyAgent = () => ({ username: '', password: '', nickname: '', email: '', phone: '', room_code: '', rebate_rate: 0, remark: '', status: 1 })
...
export function AgentsPage() {
  ...
  const openEdit = (agent: AgentItem) => { setForm({ username: agent.username, password: '', nickname: agent.nickname, email: agent.email, phone: agent.phone, room_code: agent.room_code, rebate_rate: agent.rebate_rate ?? 0, remark: agent.remark, status: agent.status }); setEditing(agent) }
  const saveNew = async () => {
  ...
  const saveEdit = async () => {
    ...
    setSaving(true)
    try { await adminApi.updateAgent(editing.id, { email: form.email, nickname: form.nickname, phone: form.phone, room_code: form.room_code.trim(), rebate_rate: form.rebate_rate, remark: form.remark, status: form.status }); showMessage('代理资料、房间号与返水已保存'); setEditing(null); await load() }
  ...
  return <Box p={{ xs: 2, lg: 2.5 }}>
    ...
    <Dialog open={createOpen || Boolean(editing)} onClose={() => !saving && (setCreateOpen(false), setEditing(null))} fullWidth maxWidth="sm"><DialogTitle>{editing ? `编辑代理 · ${editing.username}` : '新增代理账号'}</DialogTitle><DialogContent><Alert severity="info" sx={{ mt: 1, mb: 2 }}>{editing ? '房间号修改后，新的用户需输入新号进入；已有成员和客服会话仍归属该代理。' : '创建后可直接使用“登录账号 + 初始密码”登录会员端，房间号立即开通。'}</Alert><Stack gap={1.5}>{!editing && <Stack direction={{ xs: 'column', sm: 'row' }} gap={1.5}><TextField required fullWidth label="登录账号" value={form.username} onChange={event => setForm(current => ({ ...current, username: event.target.value }))} helperText="3–50 个字符，不可重复" /><TextField required fullWidth type="password" label="初始登录密码" value={form.password} onChange={event => setForm(current => ({ ...current, password: event.target.value }))} helperText="至少 6 个字符" /></Stack>}<Stack direction={{ xs: 'column', sm: 'row' }} gap={1.5}><TextField fullWidth label="代理昵称" value={form.nickname} onChange={event => setForm(current => ({ ...current, nickname: event.target.value }))} /><TextField required fullWidth label="房间号" value={form.room_code} onChange={event => setForm(current => ({ ...current, room_code: event.target.value.replace(/\D/g, '') }))} helperText="4–12 位数字，必须唯一" /></Stack><Stack direction={{ xs: 'column', sm: 'row' }} gap={1.5}><TextField fullWidth label="手机号" value={form.phone} onChange={event => setForm(current => ({ ...current, phone: event.target.value }))} /><TextField fullWidth type="email" label="邮箱" value={form.email} onChange={event => setForm(current => ({ ...current, email: event.target.value }))} /></Stack><TextField fullWidth type="number" label="房间默认返水比例 %" value={form.rebate_rate} onChange={event => setForm(current => ({ ...current, rebate_rate: Number(event.target.value) }))} inputProps={{ min: 0, max: 100, step: 0.01 }} /><TextField label="运营备注" multiline minRows={2} value={form.remark} onChange={event => setForm(current => ({ ...current, remark: event.target.value }))} /><Stack direction="row" alignItems="center" gap={1}><Switch checked={form.status === 1} onChange={(_, checked) => setForm(current => ({ ...current, status: checked ? 1 : 0 }))} /><Typography fontSize={13}>{form.status === 1 ? '账号正常启用' : '账号创建后停用'}</Typography></Stack></Stack></DialogContent><DialogActions><Button disabled={saving} onClick={() => { setCreateOpen(false); setEditing(null) }}>取消</Button><Button variant="contained" disabled={saving} onClick={() => void (editing ? saveEdit() : saveNew())}>{saving ? '保存中…' : '保存并开通'}</Button></DialogActions></Dialog>
