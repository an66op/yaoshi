...
export function ApplicationsPage({ initialCategory = 'join' }: { initialCategory?: ApplicationCategory }) {
  ...
  const submitReview = async () => {
    ...
    try {
      ...
      showMessage(decision === 'approved' ? reviewItem.request_type === 'join' ? '入房申请已通过' : '申请已通过并完成账户处理' : '申请已拒绝')
      void load()
    } catch (reason) {
  ...
  const statCards = useMemo(() => {
    ...
    return joinOnly ? reviewCards : [...reviewCards, ['今日申请金额', money(stats?.today_amount ?? 0), PaymentsRounded, '#4f7edc'] as const]
  }, [joinOnly, stats])
  const moneyRequest = reviewItem?.request_type === 'credit' || reviewItem?.request_type === 'debit'
  const detailMoneyRequest = detail?.request_type === 'credit' || detail?.request_type === 'debit'
