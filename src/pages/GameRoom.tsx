...
type SubmittedBet = { numbers: number[]; options: string[] }

const quickKeys = ['大', '1', '2', '3', '←', '小', '4', '5', '6', '龙', '单', '7', '8', '9', '冠亚', '双', '/ #', '0', '确认', '虎']
...
export function GameRoom({ game, games, theme, onBack, onOpenGame, onOpenService }: Props) {
  ...
  const [submittedBets, setSubmittedBets] = useState<SubmittedBet[]>([])
  ...
  const appendNumber = (number: number) => setSelected((current) => [...current, number])
  const appendOption = (option: string) => setSelectedOptions((current) => [...current, option])
  const clearSelection = () => { setSelected([]); setSelectedOptions([]) }
  const removeNumber = () => setSelected((numbers) => numbers.slice(0, -1))
  const submitBet = () => {
    if (!selected.length && !selectedOptions.length) return setDialog('required')
    setSubmittedBets((bets) => [...bets, { numbers: [...selected], options: [...selectedOptions] }])
  ...
  const selectionText = [...selectedOptions, ...selected].join('、')
  ...
  return <main className={`game-room theme-${theme}`} onClick={() => { setShowKeyboard(false); setShowAddMenu(false) }}>
    ...
    <section className="bet-chat" ref={chatRef}><p>以上全接，以下无效。</p><div className="admin-message"><span className="service-logo">七</span><div><small>开奖助手 · 系统通知</small><article><b>【{game.title} - {previousPeriod}】</b><hr />当前期号正在受理中<br /><br />请按下方快捷面板选择号码及投注内容</article></div></div><BetFeed game={game} />{submittedBets.map((bet, index) => <div className="player-bet" key={`${bet.options.join('-')}-${bet.numbers.join('-')}-${index}`}><div><small>林先生 · 刚刚</small><article><b>【{game.title} · 第 {game.period} 期】</b><br />投注内容：{[...bet.options, ...bet.numbers].join('、')}<br />模拟注单已发送</article></div><Avatar className="player-avatar" index={-1} label="我的头像" /></div>)}</section>
    {showKeyboard ? <BetKeyboard mode={betMode} selectedCount={selected.length + selectedOptions.length} onBackspace={removeNumber} onClear={clearSelection} onConfirm={submitBet} onModeChange={setBetMode} onSelectNumber={appendNumber} onSelectOption={appendOption} showModes={false} /> : <QuickActions onCustomerService={onOpenService} onQuickBet={() => { setShowKeyboard(false); setShowQuickBet(true) }} onSwitchGame={() => setShowGameSwitcher(true)} />}
    <section className="ticket-strip" onClick={(event) => event.stopPropagation()}><button aria-label={showKeyboard ? '收起快捷键盘' : '打开快捷键盘'} className="ticket-ime" onClick={() => { setShowAddMenu(false); setShowKeyboard((visible) => !visible) }}><img alt="" src="/icons/lucide/keyboard.svg" /></button><button aria-label="打开投注面板" className="ticket-selection" onClick={() => { setShowAddMenu(false); setShowKeyboard(true) }}>{selectionText || '点击输入投注内容'}</button><button aria-expanded={showAddMenu} aria-label="打开更多功能" className="ticket-add" onClick={() => { setShowKeyboard(false); setShowAddMenu((visible) => !visible) }}><Icon name="plus" /></button></section>
    {showAddMenu && <AddMenu onSelect={() => { setShowAddMenu(false); setDialog('assist') }} />}
    {showQuickBet && <FullBetBoard game={game} mode={betMode} selectedCount={selected.length + selectedOptions.length} onBackspace={removeNumber} onClear={clearSelection} onClose={() => setShowQuickBet(false)} onConfirm={submitBet} onModeChange={setBetMode} onSelectNumber={appendNumber} onSelectOption={appendOption} />}
...
function BetKeyboard({ mode, selectedCount, onModeChange, onBackspace, onClear, onSelectNumber, onSelectOption, onConfirm, showModes }: { mode: BetMode; selectedCount: number; onModeChange: (mode: BetMode) => void; onBackspace: () => void; onClear: () => void; onSelectNumber: (number: number) => void; onSelectOption: (option: string) => void; onConfirm: () => void; showModes: boolean }) {
  ...
  const selectQuick = (key: string) => {
    ...
    if (quickOptions.has(key)) onSelectOption(key)
  }
  const startDelete = () => {
