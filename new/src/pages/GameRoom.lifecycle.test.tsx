...
describe('room keyboard and issue lifecycle', () => {
  ...
  it('keeps an unknown game readable and normal chat usable while blocking wagers and the detailed board', async () => {
    ...
    expect(visibleText(blockedRoom)).toContain('暂未配置完整玩法')
  ...
  it('recognizes version-bound PC28, shows three balls with their sum, and opens its typed detail board', async () => {
    const pc28: Game = { ...game, id: 'canada-28', title: '加拿大28', category: 'PC28', rulesReady: true, ruleVersion: 'pc28-v2', period: '20260902001', latestIssue: '20260902000', balls: [9, 1, 9] }
    const quotes = [
    ...
    runtime.gameOdds.mockResolvedValue({ game_id: pc28.id, game_name: pc28.title, show_odds: true, rules_ready: true, rule_version: 'pc28-v2', items: quotes })
    runtime.assistantStatus.mockResolvedValue({ game_id: pc28.id, rules_ready: true, rule_version: 'pc28-v2' })
    runtime.webPlaceBatch.mockResolvedValue({ game_id: pc28.id, game_name: pc28.title, issue: pc28.period, content: '网投 1 注', lines: [], bet_count: 1, total: 20, balance: 980, accepted_at: '2026-09-02T06:45:02Z', rule_version: 'pc28-v2' })
  ...
  it('reloads stale odds when the catalog publishes a new rule version', async () => {
    ...
    expect(visibleText(render(blocked))).toContain('该彩种尚未配置完整玩法')
    ...
    expect(visibleText(tree)).not.toContain('该彩种尚未配置完整玩法')
  })
  ...
  it('retains an assistant rules denial across a rollover until the new status is verified', async () => {
    ...
    render(); await settle(); render()
    expect(visibleText(render())).toContain('玩法核验未通过')
    ...
    expect(visibleText(tree)).toContain('玩法核验未通过')
  ...
  it('does not let a delayed older status overwrite a newer rules denial', async () => {
    ...
    expect(visibleText(render({ game: next }))).toContain('新状态暂停受理')
  ...
  it('requires explicit verified readiness to clear an earlier assistant rules denial', async () => {
    ...
    const next = { ...game, period: '34137154' }
    render({ game: next }); await settle()
    expect(visibleText(render({ game: next }))).toContain('玩法核验未通过')
    ...
    expect(visibleText(tree)).not.toContain('玩法核验未通过')
