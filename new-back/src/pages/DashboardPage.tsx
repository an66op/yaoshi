import { Alert, Box, Button, Card, CardContent, Chip, CircularProgress, FormControlLabel, IconButton, InputAdornment, Paper, Stack, Switch, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, TextField, ToggleButton, ToggleButtonGroup, Typography } from '@mui/material'
...
export function DashboardPage() {
  ...
  const load = useCallback(async (notify = false) => { setError(''); setLoading(true); try { setData(await adminApi.dashboard()); setUpdatedAt(new Date()); if (notify) showMessage('运营数据已刷新') } catch (reason) { setError(reason instanceof Error ? reason.message : '服务连接失败') } finally { setLoading(false) } }, [showMessage])
