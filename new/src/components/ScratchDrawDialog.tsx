...
export function ScratchDrawDialog({ game, draw, onClose }: { game: Game; draw?: DrawResult; onClose: () => void }) {
  ...
  return <ActionDialog title="涂抹开奖" description={result ? `${game.title} · 第 ${result.issue} 期` : game.title} confirmLabel="关闭" onClose={onClose}>
    <div ref={contentRef}>{result ? <ScratchSurface key={result.key} numbers={result.numbers} /> : <p className="scratch-draw-empty" role="status">等待开奖结果</p>}</div>
  </ActionDialog>
}

function ScratchSurface({ numbers }: { numbers: number[] }) {
  ...
  return <section className="scratch-draw-board">
    ...
    <div ref={surfaceRef} className={`scratch-draw-surface${ready ? ' is-ready' : ''}${revealed ? ' is-revealed' : ''}`}>
      <div className="scratch-draw-grid" style={{ '--scratch-columns': columns } as CSSProperties} aria-hidden={!revealed} aria-label={revealed ? `开奖号码：${numbers.join('、')}` : undefined}>
        {numbers.map((number, index) => <span className="scratch-draw-cell" key={index}><b className={ballTone(number)}>{number}</b><small>{index + 1}</small></span>)}
      </div>
    ...
    {revealed && numbers.length === 3 && <p className="scratch-draw-total">{numbers.join(' + ')} = <b>{total}</b><span>{total >= 14 ? '大' : '小'} · {total % 2 ? '单' : '双'}</span></p>}
