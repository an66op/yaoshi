...
export function UsersPage() {
  ...
  const exportCsv = () => {
    const rows = users.map(user => [user.id, user.username, user.nickname, roleLabels[user.role], user.email, user.phone, user.balance.toFixed(2), user.status === 1 ? '启用' : '停用', dateTime(user.created_at)])
    const csv = [['ID', '用户名', '昵称', '角色', '邮箱', '手机', '余额', '状态', '创建时间'], ...rows].map(row => row.join(',')).join('\n')
