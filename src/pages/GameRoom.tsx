...
type SubmittedBet = { content: string }
...
export function GameRoom({ game, games, theme, onBack, onOpenGame, onOpenService }: Props) {
  ...
  const submitBet = () => {
    ...
    setSubmittedBets((bets) => [...bets, { content }])
  ...
  return <main className={`game-room theme-${theme}`} onClick={() => { setShowKeyboard(false); setShowAddMenu(false) }}>
    ...
    <section className="ticket-strip" onClick={(event) => event.stopPropagation()}><button aria-label={showKeyboard ? '收起快捷键盘' : '打开快捷键盘'} className="ticket-ime" onClick={() => { setShowAddMenu(false); setShowKeyboard((visible) => !visible) }}><img alt="" src="/icons/lucide/keyboard.svg" /></button><input aria-label="投注内容" className="ticket-selection" inputMode="text" onChange={(event) => setBetInput(event.target.value.replace(/[，,]/g, ''))} onFocus={() => { setShowAddMenu(false); setShowKeyboard(true) }} placeholder="12345/1000#3大/2000" value={betInput} /><button aria-expanded={showAddMenu} aria-label="打开更多功能" className="ticket-add" onClick={() => { setShowKeyboard(false); setShowAddMenu((visible) => !visible) }}><Icon name="plus" /></button></section>
