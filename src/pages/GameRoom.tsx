...
export function GameRoom({ game, games, theme, onBack, onOpenGame, onOpenService }: Props) {
  ...
  return <main className={`game-room theme-${theme}`} onClick={() => { setShowKeyboard(false); setShowAddMenu(false) }}>
    ...
    <section className="ticket-strip" onClick={(event) => event.stopPropagation()}><button aria-label={showKeyboard ? '收起快捷键盘' : '打开快捷键盘'} className="ticket-ime" onClick={() => { setShowAddMenu(false); setShowKeyboard((visible) => !visible) }}><img alt="" src="/icons/lucide/keyboard.svg" /></button><button aria-label="打开投注键盘" className="ticket-selection" onClick={() => { setShowAddMenu(false); setShowKeyboard((visible) => !visible) }}>{betInput || '12345/1000#3大/2000'}</button><button aria-expanded={showAddMenu} aria-label="打开更多功能" className="ticket-add" onClick={() => { setShowKeyboard(false); setShowAddMenu((visible) => !visible) }}><Icon name="plus" /></button></section>
