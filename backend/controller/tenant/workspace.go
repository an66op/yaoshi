...
// blockedSubordinateRoomBusiness keeps the old in-package implementations
// fail-closed while they are removed from the public handler surface. These
// methods are unexported below, so the route registry cannot mount them; this
// guard also prevents an accidental in-package call from crossing into an
// agent's business data.
func (h *WorkspaceHandler) blockedSubordinateRoomBusiness(c *gin.Context) (user.User, user.User, bool) {
	constants.SendError(c, http.StatusForbidden, "租户只能管理代理账号与房间资料，不能进入代理房间业务", nil)
	return user.User{}, user.User{}, false
}
...
func (h *WorkspaceHandler) blockedRoomDashboard(c *gin.Context) {
	_, agent, ok := h.blockedSubordinateRoomBusiness(c)
	if !ok {
		return
	}
...
func (h *WorkspaceHandler) blockedRoomUsers(c *gin.Context) {
	_, agent, ok := h.blockedSubordinateRoomBusiness(c)
	if !ok {
		return
	}
...
func (h *WorkspaceHandler) blockedSetRoomUserStatus(c *gin.Context) {
	_, agent, ok := h.blockedSubordinateRoomBusiness(c)
	if !ok {
		return
	}
...
func (h *WorkspaceHandler) blockedAdjustRoomUserBalance(c *gin.Context) {
	tenant, agent, ok := h.blockedSubordinateRoomBusiness(c)
	if !ok {
		return
	}
	userID, err := services.ParseUserID(c.Param("userID"))
...
func (h *WorkspaceHandler) blockedRoomBets(c *gin.Context) {
	_, agent, ok := h.blockedSubordinateRoomBusiness(c)
	if !ok {
		return
	}
...
func (h *WorkspaceHandler) blockedRoomApplications(c *gin.Context) {
	_, agent, ok := h.blockedSubordinateRoomBusiness(c)
	if !ok {
		return
	}
...
func (h *WorkspaceHandler) blockedReviewRoomApplication(c *gin.Context) {
	tenant, agent, ok := h.blockedSubordinateRoomBusiness(c)
	if !ok {
		return
	}
	...
	result, err := h.work.ReviewApplication(agent.UserID, applicationID, services.ReviewApplicationInput{Decision: request.Decision, ReceivedAmount: request.ReceivedAmount, OddsMultiplier: request.OddsMultiplier, Remark: request.Remark, Operator: "租户 " + tenant.Username})
	if err != nil {
		constants.SendError(c, http.StatusBadRequest, "审核申请失败", err)
		return
	}
...
func (h *WorkspaceHandler) blockedRoomOperatingReport(c *gin.Context) {
	_, agent, ok := h.blockedSubordinateRoomBusiness(c)
	if !ok {
		return
	}
...
func (h *WorkspaceHandler) blockedRoomConversations(c *gin.Context) {
	_, agent, ok := h.blockedSubordinateRoomBusiness(c)
	if !ok {
		return
	}
...
func (h *WorkspaceHandler) blockedRoomMessages(c *gin.Context) {
	_, agent, ok := h.blockedSubordinateRoomBusiness(c)
	if !ok {
		return
	}
...
func (h *WorkspaceHandler) blockedReplyRoomChat(c *gin.Context) {
	tenant, agent, ok := h.blockedSubordinateRoomBusiness(c)
	if !ok {
		return
	}
...
func (h *WorkspaceHandler) blockedSendRoomRedPacket(c *gin.Context) {
	tenant, agent, ok := h.blockedSubordinateRoomBusiness(c)
	if !ok {
		return
	}
	...
	result, err := h.chat.SendRedPacket(roomScope, roomScope, request.GameID, services.ChatRedPacketInput{RequestID: request.RequestID, Count: request.Count, TotalAmount: request.TotalAmount, MinDailyTurnover: request.MinDailyTurnover, Greeting: request.Greeting, Cover: request.Cover}, firstTenantName(tenant))
	if err != nil {
		constants.SendError(c, http.StatusBadRequest, "发送红包失败", err)
		return
	}
...
func (h *WorkspaceHandler) blockedRoomRobotStatus(c *gin.Context) {
	if _, _, ok := h.blockedSubordinateRoomBusiness(c); !ok {
		return
	}
	constants.SendSuccess(c, http.StatusOK, "ok", services.RoomActivityStatusSnapshot())
}
func (h *WorkspaceHandler) blockedRunRoomRobot(c *gin.Context) {
	_, agent, ok := h.blockedSubordinateRoomBusiness(c)
	if !ok {
		return
	}
...
func (h *WorkspaceHandler) blockedSetLotteryRoomStatus(c *gin.Context) {
	_, agent, ok := h.blockedSubordinateRoomBusiness(c)
	if !ok {
		return
	}
