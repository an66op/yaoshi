...
for command_name in psql curl awk mv basename dirname mkdir rmdir; do command -v "$command_name" >/dev/null 2>&1 || { echo "缺少命令：$command_name" >&2; exit 1; }; done
...
for key in status database backup backup_sha256 scope server_system_identifier server_address server_port sentinel_token_sha256; do
